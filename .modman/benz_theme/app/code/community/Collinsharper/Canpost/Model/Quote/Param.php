 ...
 * @author      Maxim Nulman
 */
class Collinsharper_Canpost_Model_Quote_Param extends Mage_Core_Model_Abstract
{
    
    
    protected function _construct() {
        
        $this->_init('chcanpost2module/quote_param');
     ...
     * @param type $do_not_safe_drop
     * @param type $leave_at_door
     * @param type $cp_office_id
     * @param type $est_delivery_date
                                  ...
                                  )
    {
        ...
        // KL: Fix the FK issue
        try {
            $item->setMagentoQuoteId(999);
            $item->save();
        } catch (Exception $e) {
            if ($e->getCode() == '23000') {
                Mage::helper('chcanpost2module')->log('Quote ID: ' . $quote_id . ' does not exists' . "\r\n\r\n" . $e->getMessage());
            } else {
    ...
    public function getByOrderId($order_id)
    {
        ...
        return $this->getCollection()->addFieldToFilter('magento_order_id', $order_id)->getFirstItem();
        
    }
    
    
    public function getParamsByQuote($quote_id, $data = array(), $type = 'quote')
    {
        
        $params = ($type == 'quote') ? $this->getByQuoteId($quote_id) : $this->getByOrderId($quote_id);
